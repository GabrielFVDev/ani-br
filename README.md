...
Por padrão `sources="anroll animefire"`. Dá pra reordenar ou restringir com a env `ANI_CLI_SOURCES` (veja abaixo).

> O AnimeFire pode retornar **HTTP 401** no vídeo dependendo do IP/região (a busca funciona, mas o vídeo é bloqueado). Por isso o AnimesROLL vem primeiro.
